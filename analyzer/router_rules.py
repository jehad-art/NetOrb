    ...
    "misconfigurations": [
        {
            ...
            "check": lambda d: any("line vty" in l for b in d["parsed"].get("vty", []) for l in b)
                              and not any("access-class" in l for b in d["parsed"].get("vty", []) for l in b)
        },
        {
            "id": "R06",
            "tag": "empty_acl",
            ...
            "check": lambda d: any(len(acl.get("entries", [])) == 0 for acl in d["access_lists"])
        },
        {
            "id": "R07",
            "tag": "acl_no_deny_end",
            ...
            "check": lambda d: any(not acl["entries"][-1]["match"].lower().startswith("deny")
                                   for acl in d["access_lists"] if acl.get("entries"))
        },
        {
            "id": "R08",
            "tag": "acl_permit_any",
            ...
            "check": lambda d: any("any" in e["match"].lower() or "0.0.0.0" in e["match"]
                                   for acl in d["access_lists"] for e in acl.get("entries", []))
        },
        {
            "id": "R09",
            "tag": "nat_no_overload",
            ...
            "check": lambda d: any(not r.get("overload") for r in d["parsed"].get("nat", []))
        },
        {
            "id": "R10",
            ...
            "check": lambda d: any(r.get("acl") not in [a.get("name") for a in d["access_lists"]]
                                   for r in d["parsed"].get("nat", []))
        },
        {
            "id": "R11",
        ...
        },
        {
            ...
            "check": lambda d: any("no logging buffered" in l for l in d["raw"])
        },
        {
            "id": "R15",
            ...
            "check": lambda d: any("no logging console" in l for l in d["raw"])
        },
        {
            "id": "R16",
            ...
            "check": lambda d: any("username " in l and " password " in l and "secret" not in l for l in d["raw"])
            and not any("service password-encryption" in s for s in d["parsed"].get("services", []))
        },
    ],
    "missing_recommendations": [
        {
            ...
            "check": lambda d: not d["parsed"].get("usernames", [])
        },
        {
            "id": "R20",
            ...
            "check": lambda d: not d["parsed"].get("aaa")
        },
        {
            "id": "R21",
            ...
            "check": lambda d: not any("logging buffered" in l for l in d["raw"])
        },
        {
            "id": "R22",
